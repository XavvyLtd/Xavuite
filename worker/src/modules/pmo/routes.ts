...
export async function handlePMO(
  ...
  // Hard delete — but only if the project has zero linked records.
  // Foreign keys aren't enforced at the DB level in this worker, so a
  // naive DELETE would silently orphan rows in every table below rather
  // than fail loudly. This checks each one explicitly first and refuses
  // if anything is still attached, telling the caller exactly what's
  // blocking it so they can decide whether to clean up or cancel instead.
  if (resource === 'projects' && id && method === 'DELETE') {
    const denied = requirePermission(ctx, 'pmo:manage:project');
    if (denied) return denied;

    const project = await env.DB.prepare(
      `SELECT id, name FROM pmo_projects WHERE id = ? AND tenant_id = ?`
    ).bind(id, ctx.tenantId).first() as any;
    if (!project) return err('Project not found', 404);

    const linkedCounts = await Promise.all([
      env.DB.prepare(`SELECT COUNT(*) AS n FROM pmo_sprints WHERE project_id = ?`).bind(id).first(),
      env.DB.prepare(`SELECT COUNT(*) AS n FROM pmo_tasks WHERE project_id = ?`).bind(id).first(),
      env.DB.prepare(`SELECT COUNT(*) AS n FROM pmo_allocations WHERE project_id = ?`).bind(id).first(),
      env.DB.prepare(`SELECT COUNT(*) AS n FROM resource_bookings WHERE project_id = ?`).bind(id).first(),
      env.DB.prepare(`SELECT COUNT(*) AS n FROM timesheet_entries WHERE project_id = ?`).bind(id).first(),
      env.DB.prepare(`SELECT COUNT(*) AS n FROM overtime_records WHERE project_id = ?`).bind(id).first(),
    ]) as any[];

    const labels = ['sprints', 'tasks', 'allocations', 'resource bookings', 'timesheet entries', 'overtime records'];
    const blocking = labels
      .map((label, i) => ({ label, count: linkedCounts[i]?.n ?? 0 }))
      .filter(b => b.count > 0);

    if (blocking.length > 0) {
      const summary = blocking.map(b => `${b.count} ${b.label}`).join(', ');
      return err(
        `Cannot delete "${project.name}" — it still has linked data: ${summary}. ` +
        `Remove these first, or set the project's status to "cancelled" instead of deleting it.`,
        409
      );
    }

    await env.DB.prepare(`DELETE FROM pmo_projects WHERE id = ? AND tenant_id = ?`).bind(id, ctx.tenantId).run();

    await audit(env, {
      ...auditFromRequest(request, ctx),
      action: 'delete', resource: 'pmo_project', resourceId: id,
    });
