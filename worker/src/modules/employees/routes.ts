...
export async function handleEmployees(
  ...
  if (id && !action && request.method === 'PATCH') {
    ...
    // Email lives on users, not employee_history — update it separately if changed.
    // employee_history never stores email, so without this block any email sent
    // via PATCH /api/employees/:id was silently accepted by the schema and dropped.
    ...
    if (d.email) {
      ...
      if (empRow?.user_id) {
        ...
        ).bind(empRow.user_id).first() as any;
        ...
        if (currentUser && currentUser.email !== newEmail) {
          ...
          ).bind(newEmail, ctx.tenantId, empRow.user_id).first();
          if (clash) return err('Email is already in use by another user', 409);

          await env.DB.prepare(
            `UPDATE users SET email = ? WHERE id = ?`
          ).bind(newEmail, empRow.user_id).run();

          emailChange = { from: currentUser.email, to: newEmail };
        }
      }
    }
    ...
    await audit(env, {
      ...
      changes: {
        ...Object.fromEntries(
          Object.entries(d)
            .filter(([k]) => k !== 'changeReason' && k !== 'email')
