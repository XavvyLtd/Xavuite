...
export async function handleResources(
  ...
  if (resource === 'capacity' && request.method === 'GET') {
    ...
    const weekStart = url.searchParams.get('weekStart') ?? getMonday(new Date());
    ...
    const rows = await env.DB.prepare(`
      SELECT
        ...
        -- Booked hours from resource bookings (planned)
        COALESCE((
        ...
        ), 0) / 37.5 * 100, 1) AS utilisation_pct,
    ...
    `).bind(weekStart, weekEnd, weekStart, weekEnd, weekStart, weekEnd, weekStart, weekEnd, ctx.tenantId).all();
  ...
  if (resource === 'forecast' && request.method === 'GET') {
    ...
    const rows  = await env.DB.prepare(`
      ...
      WHERE rb.tenant_id=? AND rb.week_starting >= date('now','weekday 1','-7 days')
        AND rb.week_starting <= date('now','weekday 1','+${weeks} weeks')
