...
import { useSettings, useUpdateBranding, useModuleSettings,
         ...
         useSSORProviders, useSaveSSORProvider,
         useStatutorySettings, useUpdateStatutorySettings } from '../../hooks/api';
...
export default function SettingsModule() {
  const [tab, setTab] = useState<'branding'|'company'|'modules'|'sso'|'mfa'>('branding');
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div style={{display:'flex',gap:6,marginBottom:20,flexWrap:'wrap'}}>
        {[{k:'branding',l:'🎨 Branding'},{k:'company',l:'🏢 Company & Statutory'},{k:'modules',l:'🧩 Modules'},{k:'sso',l:'🔐 SSO / Identity'},{k:'mfa',l:'📱 MFA'}].map(t=>(
...
function CompanyStatutoryTab() {
  const { data, isLoading } = useStatutorySettings();
  const update = useUpdateStatutorySettings();
  ...
  useEffect(() => {
    if (data) {
      setForm(data);
      const mins = (data as any).session_timeout_minutes ?? 60;
      const preset = SESSION_PRESETS.find(p => p.value === mins);
      if (preset && preset.value !== -1) { setSessionPreset(mins); }
      else { setSessionPreset(-1); setCustomMins(mins); }
    }
  }, [data]);

  const set = (k: string, v: any) => setForm((f: any) => ({ ...f, [k]: v }));
  const effectiveTimeout = sessionPreset === -1 ? customMins : sessionPreset;

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await update.mutateAsync({ ...form, session_timeout_minutes: effectiveTimeout } as any);
      setSaved(true); setTimeout(() => setSaved(false), 3000);
    } catch {}
  };
  ...
  const Sect = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <Card style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: C.dim, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>{title}</div>
  ...
  return (
    <form onSubmit={handleSave} style={{ maxWidth: 700 }}>
      <Sect title="Company registration">
        <FormField label="Company reg. number">
          <input style={inputStyle} value={form.company_reg_number ?? ''} onChange={e => set('company_reg_number', e.target.value)} placeholder="e.g. 12345678" />
        </FormField>
        <FormField label="VAT number">
          <input style={inputStyle} value={form.vat_number ?? ''} onChange={e => set('vat_number', e.target.value)} placeholder="e.g. GB123456789" />
        </FormField>
        <FormField label="Tax / UTR reference">
          <input style={inputStyle} value={form.tax_reference ?? ''} onChange={e => set('tax_reference', e.target.value)} />
        </FormField>
      </Sect>

      <Sect title="Registered address">
        <Full>
          <FormField label="Address line 1">
            <input style={inputStyle} value={form.reg_address_line1 ?? ''} onChange={e => set('reg_address_line1', e.target.value)} />
          </FormField>
        </Full>
        <Full>
          <FormField label="Address line 2">
            <input style={inputStyle} value={form.reg_address_line2 ?? ''} onChange={e => set('reg_address_line2', e.target.value)} />
          </FormField>
        </Full>
        ...
        <FormField label="Country"><input style={inputStyle} value={form.reg_country ?? 'United Kingdom'} onChange={e => set('reg_country', e.target.value)} /></FormField>
      </Sect>

      <Sect title="Invoice defaults">
        <FormField label="Default currency">
          <select style={selectStyle} value={form.default_currency ?? 'GBP'} onChange={e => set('default_currency', e.target.value)}>
        ...
        <FormField label="Default VAT rate (%)">
          <input style={inputStyle} type="number" min={0} max={100} step={0.5} value={form.default_tax_rate ?? 20} onChange={e => set('default_tax_rate', +e.target.value)} />
        </FormField>
        <FormField label="Default payment terms (days)">
          <input style={inputStyle} type="number" min={0} max={365} value={form.payment_terms_days ?? 30} onChange={e => set('payment_terms_days', +e.target.value)} />
        </FormField>
      </Sect>

      <Sect title="Bank details (printed on invoices)">
        <FormField label="Bank name"><input style={inputStyle} value={form.bank_name ?? ''} onChange={e => set('bank_name', e.target.value)} /></FormField>
        <FormField label="Account name"><input style={inputStyle} value={form.bank_account_name ?? ''} onChange={e => set('bank_account_name', e.target.value)} /></FormField>
        <FormField label="Account number"><input style={inputStyle} value={form.bank_account_number ?? ''} onChange={e => set('bank_account_number', e.target.value)} /></FormField>
        <FormField label="Sort code"><input style={inputStyle} value={form.bank_sort_code ?? ''} onChange={e => set('bank_sort_code', e.target.value)} placeholder="00-00-00" /></FormField>
        <FormField label="IBAN"><input style={inputStyle} value={form.bank_iban ?? ''} onChange={e => set('bank_iban', e.target.value)} /></FormField>
        <FormField label="BIC / SWIFT"><input style={inputStyle} value={form.bank_bic ?? ''} onChange={e => set('bank_bic', e.target.value)} /></FormField>
      </Sect>

      <Card style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.dim, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Session timeout</div>
        <div style={{ fontSize: 12, color: C.muted, marginBottom: 14 }}>
          How long before an idle session requires re-login. A warning appears 60 seconds before expiry so users can save their work.
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
          {SESSION_PRESETS.map(p => (
            <button key={p.value} type="button"
              onClick={() => { setSessionPreset(p.value); if (p.value !== -1) setCustomMins(p.value); }}
              style={{
                padding: '6px 14px', borderRadius: 20, border: `1px solid ${sessionPreset === p.value ? C.primary : C.border}`,
                background: sessionPreset === p.value ? C.primary : C.elevated,
                color: sessionPreset === p.value ? '#fff' : C.muted,
        ...
        {sessionPreset === -1 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <input style={{ ...inputStyle, width: 90 }} type="number" min={5} max={1440} value={customMins} onChange={e => setCustomMins(+e.target.value)} />
            <span style={{ fontSize: 12, color: C.muted }}>minutes</span>
          </div>
        )}
        <div style={{ fontSize: 11, color: C.dim, marginTop: 10 }}>
          Effective timeout: <strong style={{ color: C.text }}>{effectiveTimeout} minutes</strong>
        </div>
      </Card>

      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <button type="submit" disabled={update.isPending} style={{ background: C.primary, color: '#fff', border: 'none', borderRadius: 8, padding: '9px 24px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
