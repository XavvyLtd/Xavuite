...
// ── Cron handler ──────────────────────────────────────────────────────────────
// Single cron fires every 30 minutes. Logic checks time/day internally.
// wrangler.toml only needs: crons = ["*/30 * * * *"]
async function handleCron(event: ScheduledEvent, env: Env): Promise<void> {
  ...
  console.log(`[cron] Fired at ${now.toISOString()} day=${day} hour=${hour} min=${min}`);

  // DB-driven jobs — runs any job whose next_run is due
  await runScheduledJobs(env, '*/30 * * * *');

  // Process SLA breaches on every run
  await processSLABreaches(env, env.TENANT_ID);

  try {
    // Monday ~6am UTC: weekly compliance check
    if (day === 1 && hour === 6 && min < 30) {
    ...
    // Friday ~5pm UTC: sprint closeout + activate next sprint tasks
    if (day === 5 && hour === 17 && min < 30) {
    ...
    // 26th of month ~9am UTC: timesheet reminder
    if (date === 26 && hour === 9 && min < 30) {
