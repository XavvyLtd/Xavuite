...
export async function handleDashboard(
  ...
  if (!resource && request.method === 'GET') {
    ...
    const [
      ...
      recentActivity, invoicesDue, avgUtilisation,
    ] = await Promise.all([
      ...
      `).bind(tid).all() as any,
      env.DB.prepare(`
        SELECT COUNT(*) as n, SUM(total) as value
        FROM invoices WHERE tenant_id=? AND status='sent'
          AND due_date <= date('now', '+30 days') AND due_date >= date('now')
      `).bind(tid).first() as any,
      env.DB.prepare(`
        SELECT ROUND(AVG(rb.hours) / 37.5 * 100, 1) as pct
        FROM resource_bookings rb
        JOIN employees e ON e.id = rb.employee_id AND e.status = 'active'
        WHERE rb.tenant_id = ? AND rb.week_starting >= date('now','weekday 1','-28 days')
          AND rb.week_starting <= date('now')
      `).bind(tid).first() as any,
    ]);
    ...
    return ok({
      metrics: {
        ...
        invoicesDueSoon:      invoicesDue?.n         ?? 0,
        invoicesDueValue:     invoicesDue?.value      ?? 0,
        avgUtilisation:       avgUtilisation?.pct     ?? 0,
      },
