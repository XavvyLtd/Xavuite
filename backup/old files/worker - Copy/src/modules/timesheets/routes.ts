...
const EntrySchema = z.object({
  ...
  taskId:      z.string().optional(),   // ← new
});

const CreateTimesheetSchema = z.object({
  weekStarting: z.string().date(),
  entries:      z.array(EntrySchema).min(1).max(7),
});
...
export async function handleTimesheets(
  ...
  if (!id && request.method === 'POST') {
    ...
    // Check for existing non-rejected timesheet
    const existing = await env.DB.prepare(
      `SELECT id, status FROM timesheets WHERE employee_id = ? AND week_starting = ?`
    ).bind(employee.id, d.weekStarting).first<{ id: string; status: string }>();

    if (existing) {
      if (existing.status === 'pending' || existing.status === 'approved') {
        return err('A timesheet for this week is already ' + existing.status, 409);
      }
      // Status is 'rejected' — delete old entries and resubmit
      await env.DB.prepare(`DELETE FROM timesheet_entries WHERE timesheet_id = ?`).bind(existing.id).run();
      await env.DB.prepare(`DELETE FROM timesheets WHERE id = ?`).bind(existing.id).run();
    }
    ...
    `).bind(tsId, ctx.tenantId, employee.id, d.weekStarting).run();
    ...
    const entryStmts = d.entries.map(e =>
      env.DB.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(
        crypto.randomUUID(), tsId, ctx.tenantId,
        e.date, e.hoursWorked, e.description ?? null, e.billable ? 1 : 0,
        e.projectId ?? null, e.taskId ?? null
      )
    );
  ...
  if (id && !action && request.method === 'GET') {
    ...
    // Return entries with project + task names
    const entries = await env.DB.prepare(`
      SELECT te.*,
             p.name AS project_name,
             pt.name AS task_name
      FROM timesheet_entries te
      LEFT JOIN pmo_projects p  ON p.id  = te.project_id
      LEFT JOIN pmo_tasks    pt ON pt.id = te.task_id
      WHERE te.timesheet_id = ? AND te.tenant_id = ?
      ORDER BY te.date
